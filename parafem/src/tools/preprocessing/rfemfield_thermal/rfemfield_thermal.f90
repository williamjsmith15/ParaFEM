PROGRAM rfemfield_thermal

!/****h* tools/preprocessing/rfemfield_thermal
!*  NAME
!*    PROGRAM rfemfield_thermal
!*  FUNCTION
!*    Generates a spatially correlated log-normal random field of diffusivity
!*    (or thermal conductivity) values and applies it to a model mesh.
!*    Produces per-element material files for use with rfemsolve_thermal.
!*  USAGE
!*    rfemfield_thermal <rfem_job_name> <model_job_name> <instance-id>
!*    rfemfield_thermal <rfem_job_name>
!*  INPUTS
!*    <rfem_job_name>.rf   - Random field control file (sim3de format, no v line)
!*    <model_job_name>.dat - Model control file (rfemsolve format)
!*    <model_job_name>.d   - Model mesh file
!*  OUTPUTS
!*    <model_job_name>-<instance-id>.d   - Mesh with per-element material IDs
!*    <model_job_name>-<instance-id>.dat - Updated control (np_types=nels)
!*    <model_job_name>-<instance-id>.mat - Per-element diffusivity values
!*    <rfem_job_name>.mat                - Random field material file (optional)
!*    <rfem_job_name>.d                  - Random field mesh (optional, output=2)
!*    <rfem_job_name>.res                - Statistics summary
!*  AUTHOR
!*    Adapted from rfemfield by Lee Margetts
!*  COPYRIGHT
...
!------------------------------------------------------------------------------
  ...
  REAL(iwp), ALLOCATABLE :: efld(:,:,:)   ! diffusivity random field
...
!------------------------------------------------------------------------------
  ...
  IF( (argc /= 3) .and. (argc /= 1) ) THEN
     PRINT*
     PRINT*, "Usage:  rfemfield_thermal <rfem_job_name> <model_job_name> <instance-id>"
     PRINT*, "   Or:  rfemfield_thermal <rfem_job_name>"
     PRINT*
...
!------------------------------------------------------------------------------
  ...
  CASE('sim3de')
     ...
     READ(10,*) emn,esd
     READ(10,*) varfnc
     ! Note: no Poisson ratio line -- this is diffusivity/conductivity only

     nye    = nels/nxe/nze
     nn     = (nxe+1)*(nye+1)*(nze+1)
     nprops = nels    ! one material row per element
     np_types = 1     ! one property per element (diffusivity)
     ndim   = 3
     nod    = 8

     IF(output /= 1 .AND. output /= 2) THEN
        PRINT *, output, " = Incorrect value for output. Accepted values are '1' or '2'"
        STOP
     END IF
...
!------------------------------------------------------------------------------
     ...
     IF( do_model ) THEN
        ...
        DO i = 2,m_nn
           ...
           DO ord = 1,3
              IF ( m_coord(ord,i) < min_extents(ord) ) min_extents(ord) = m_coord(ord,i)
              IF ( m_coord(ord,i) > max_extents(ord) ) max_extents(ord) = m_coord(ord,i)
           END DO
        END DO
        READ(15,*)   ! headers
        DO iel = 1,m_nels
        ...
        END DO
        WRITE(11, '(A)') "Size of model extents:"
        WRITE(11,'(3F14.6)') size_extents

     END IF

!------------------------------------------------------------------------------
! 7. Generate spatially random field for diffusivity in a regular cuboid
!------------------------------------------------------------------------------
     ...
     WRITE(11,'(A,E12.4)') "eavg (arithmetic mean of D) =", eavg
     WRITE(11,'(A,E12.4)') "egeo (geometric  mean of D) =", egeo
     WRITE(11,'(A,E12.4)') "ehrm (harmonic   mean of D) =", ehrm
     ...
     DO i = 1, nze
        DO j = 1, nye
           DO k = 1, nxe
              iel = iel + 1
              WRITE(12,'(I8,E12.4)') iel, efld(k,j,i)
           END DO
        END DO
     END DO
...
!------------------------------------------------------------------------------
! 8. Generate finite element mesh for the rfem regular cuboid (output=2)
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
! 9. Rewrite model dataset with per-element material IDs and material file
!------------------------------------------------------------------------------
     ...
     IF( do_model ) THEN
        ...
        DO iel = 1, m_nels
           ...
           WRITE(17,'(I8,E12.4)') iel, efld(k,j,i)
        END DO
...
!------------------------------------------------------------------------------
! 10. Write updated .dat file for rfemsolve_thermal (np_types = m_nels)
!------------------------------------------------------------------------------
     ...
     IF( do_model ) THEN
        ...
        WRITE(14,*) m_nels
        WRITE(14, '(7I8)') m_nels, m_nn, m_nr, m_nip, m_nod, m_loaded_nodes, m_fixed_freedoms
